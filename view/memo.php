...
for ($i = 0; $row = mysqli_fetch_assoc($result); $i++) {
	...
	$list[$i]['del_href'] = '../lib/memo_delete.php?me_id=' . $row['me_id'] . '&amp;kind=' . $kind; // 쪽지 삭제 링크
}

$str = ''; // 페이징 시작
if ($page > 1) {
	$str .= '<a href="./memo.php?kind=' . $kind . '&amp;page=1" class="pg_page pg_start"> First </a>';
}
...
if ($start_page > 1) $str .= '<a href="./memo.php?kind=' . $kind . '&amp;page=' . ($start_page - 1) . '" class="pg_page pg_prev">Back' . $start_page - 1 . ' </a>';
...
if ($total_page > $end_page) $str .= '<a href="./memo.php?kind=' . $kind . '&amp;page=' . ($end_page + 1) . '" class="pg_page pg_next">' . $end_page + 1 . 'Next </a>';

if ($page < $total_page) {
	$str .= '<a href="./memo.php?kind=' . $kind . '&amp;page=' . $total_page . '" class="pg_page pg_end"> Last </a>';
}
					...
					<tbody>
						<?php for ($i = 0; $i < count($list); $i++) {  ?>
							<tr>
								...
								<td><a class="check_btn" href="<?php echo $list[$i]['view_href'] ?>"><?php echo $list[$i]['read_datetime'] ?></a></td>
								<td><a class="check_btn" href="<?php echo $list[$i]['del_href'] ?>" onclick="del(this.href); return false;"> 삭제 </a></td>
							</tr>
						<?php }  ?>
						<?php if ($i == 0) {
